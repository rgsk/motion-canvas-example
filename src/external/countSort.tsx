...
export default makeScene2D(function* (view) {
  ...
  const signals = [
    createSignal(randomNumbers[0]),
    createSignal(randomNumbers[1]),
    createSignal(randomNumbers[2]),
    createSignal(randomNumbers[3]),
    createSignal(randomNumbers[4]),
    createSignal(randomNumbers[5]),
    createSignal(randomNumbers[6]),
  ];
  ...
  yield* all(
    ...
    ...range(signals.length).map((i) => output_rects[i].opacity(0.5, 0.5)),
    ...range(signals.length).map((i) => output_txts[i].opacity(1, 0.5))
  );
  ...
  yield* all(
    ...
    ...range(10).map((i) => count_rects[i].opacity(0.5, 0.5)),
    ...range(10).map((i) => count_txts[i].opacity(1, 0.5))
  );
  ...
  for (let i = 0; i < signals.length; i++) {
    ...
    yield* code().selection(lines(3), 0.5),
      yield* all(
        ...
        count_txts[signals[i]()].text(
          (parseInt(count_txts[signals[i]()].text()) + 1).toString(),
          0.5
        ),
  ...
  for (let i = 1; i < 10; i++) {
    yield* code().selection(lines(5), 0.5),
      ...
      yield* all(
        ...
        count_txts[i].text(
          (
            parseInt(count_txts[i].text()) + parseInt(count_txts[i - 1].text())
          ).toString(),
          0.5
        ),
        count_rects[i].fill("#e6a700", 0.5)
      );
    yield* all(count_rects[i].fill("#e3242b", 0.5));
  }
  ...
  for (let i = signals.length - 1; i > -1; i--) {
    yield* code().selection(lines(7), 0.5),
      yield* all(
        ...
        output_txts[parseInt(count_txts[signals[i]()].text()) - 1].text(
          signals[i]().toString(),
          0.5
        ),
        output_rects[parseInt(count_txts[signals[i]()].text()) - 1].fill(
          "#2832c2",
          0.5
        )
      );
