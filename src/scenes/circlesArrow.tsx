...
export default makeScene2D(function* (view) {
  ...
  circles.forEach((ref, i) => {
    view.add(
      <Circle ref={ref} size={(1 + i) * 100} stroke={"white"} lineWidth={5} />
    );
  });
  ...
  yield* all(
    circles[0]().bottom(
      new Vector2(circles[0]().position.x(), circles[1]().bottom().y),
      1
    ),
    circles[2]().bottom(
      new Vector2(circles[2]().position.x(), circles[1]().bottom().y),
      1
    )
  );
  ...
  yield* all(
    ...circles.map((ref) => ref().top(new Vector2(ref().top().x, -300), 1))
  );
});
